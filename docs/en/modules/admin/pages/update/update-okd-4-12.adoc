...
.. Annotate the *Machine* resources within the registry’s *MachineSet* for later deletion.
...
----
+
This command sets the `istiod` deployment’s replica count to `0` in the `istio-system` namespace, temporarily stopping it.
...
* Before updating OKD, be aware that worker nodes may hang or fail to update during the process. If you notice this behavior, it’s recommended to delete the *Machine* resource for the problematic node.
...
[NOTE]
====
The secrets may regenerate after deletion — this is normal behavior.
====
...
[CAUTION]
====
After deleting the machine, it is critical to wait **2–3 minutes** before proceeding to the next step: xref:#apply-new-machine-config[Applying the new machine configuration] (*step n*).
...
+
After waiting 2–3 minutes following the node deletion, apply the new configuration file.
+
[source,bash]
----
...
.. In the follow-up steps — xref:#available-etcd-pods-status[Checking the status of the existing etcd pods],
xref:#connect-to-any-etcd-pod[Connecting to any etcd pod], and
xref:#get-etcd-member-list[Viewing the list of etcd members] —
...
Note that the `kube-apiserver` operator often shows the `Progressing` status during node revision updates — this is normal, and you should wait until it switches to `Active`, indicating that the update is complete and the cluster is stable.
+
...
.. Once connected via **SSH**, run the following command on the affected node’s terminal to restore the default security context settings on all files and directories:
+
[source,bash]
----
...
[CAUTION]
====
It’s essential to perform this process for every node that experienced issues and remains in `Not Ready` status for an extended time.
...
It’s expected that **SELinux** will automatically revert to **Enforcing** mode in the next OKD version update, specifically in *step 14*.
====
...
it’s recommended to manually restore **Enforcing** mode.
...
+
.📌 *_IP addresses. YAML configuration file for ``ingresscontroller``_*
[%collapsible]
====
[source,yaml]
----
...
If renaming — append any number or letter sequence to the old name.
+
...
+
It is recommended to bring up 1–2 registries at a time using the provided bash script, which performs the following for each registry (codebase):
+
--
...
* Issue with *Machine Config Pool* resource — the MCP for master nodes could not locate the required *Machine Config* resource.
...
* `ocs-operator` upgrade failure on OKD 4.12 — update did not start after modifying the *Subscription* resource.

== Requires further testing

* OKD upgrade on vSphere — _in progress_.
* QA testing of Install/Update scenarios (OKD, Platform, registries) on AWS — _in progress_.
* QA testing of Install/Update scenarios (OKD, Platform, registries) on vSphere.
* QA testing of backup and recovery procedures for Control Plane and registries — _in progress_.
