...
To create search conditions, *use the* `*<ext:createSearchCondition>*` *tag*, developed as part of the Liquibase-based physical data modeling tool extension on the Digital Services Development Platform, as well as specific xref:registry-develop:data-modeling/data/physical-model/liquibase-ddm-ext.adoc#sc-attribute-values[search condition attributes].
